...
Shader "Game/UI/Rough"
{
    Properties
    {
        ...
        _EdgeWidth ("Rough Edge Width (Pixels)", Range(0, 8)) = 2
        _OverlapScale ("Overlap Scale", Float) = 1
        _OverlapIntensity ("Overlap Intensity", Range(0, 1)) = 0.25
        [ToggleUI] _OverlapSmooth ("Smooth Overlap Movement", Float) = 0
        
        _OverlapMoveDuration ("Pattern Step Duration (Seconds, 0 = Still)", Float) = 0.3333333
    }
    ...
    SubShader
    {
        ...
        CGPROGRAM
            ...
            struct appdata_t
            {
                ...
                float4 spriteBounds : TEXCOORD1;
                float2 hoverData : TEXCOORD2;
            ...
            struct v2f
            {
                ...
                float4 spriteBounds : TEXCOORD3;
                float2 overlayPosition : TEXCOORD4;
                float2 hoverData : TEXCOORD5;
            ...
            v2f vert(appdata_t v)
            {
                ...
                OUT.spriteBounds = v.spriteBounds;
                OUT.hoverData = v.hoverData;
                OUT.overlayPosition = mul(unity_ObjectToWorld, v.vertex).xy;
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                half4 color = ApplyRoughUI(sprite, IN.texcoord,
                    IN.worldPosition.xy, IN.spriteBounds, IN.overlayPosition, IN.color, IN.hoverData);
        ...
        CGPROGRAM
            ...
            v2f vert(appdata_t v)
            {
                ...
                float4 vPosition = mul(_NonJitteredViewProjMatrix, mul(UNITY_MATRIX_M, v.vertex));
                OUT.positionCSNoJitter = vPosition;
                OUT.previousPositionCSNoJitter = mul(_PrevViewProjMatrix, mul(UNITY_PREV_MATRIX_M, v.oldvertex));
                OUT.worldPosition = v.vertex;
                OUT.spriteBounds = v.spriteBounds;
                OUT.hoverData = v.hoverData;
                OUT.overlayPosition = mul(unity_ObjectToWorld, v.vertex).xy;
